use riichi::agent::{BatchAgent, Tsumogiri};
use riichi::arena::game::{BatchGame, Index};
use riichi::arena::result::GameResult;
...
fn main() -> Result<()> {
    ...
    println!("Starting self-play with Tsumogiri");
    println!("Will play 2 games and save logs to {:?}", output_dir);
    
    let start_time = Instant::now();
    
    // Create agents - exactly as in the test
    let mut agents = [
        Box::new(Tsumogiri::new_batched(&[0, 1, 2, 3])?) as Box<dyn BatchAgent>,
        Box::new(Tsumogiri::new_batched(&[3, 2, 1, 0])?) as Box<dyn BatchAgent>,
    ];
    
    // Create game indexes - exactly as in the test
    let indexes = &[
        [
    ...
    // Run the games - exactly as in the test
